...
for i,(x,y) in enumerate(star_loc):
    ...
    if abs(x_pixel)>=length/2 or abs(y_pixel)>=height/2:
        continue
    else:
        indexes.append(i)
...
star_in_FOV = pd.DataFrame(
    data,
    columns=['Star ID','RA','DE']
)

# star_in_FOV.to_excel('Stars.xlsx')
...
for i,coordinate1 in enumerate(dir_vectors):
    ...
    for j,coordinate2 in enumerate(dir_vectors):
        ...
        y2 = coordinate2[1]
        z2 = coordinate2[2]
        resultant = sqrt((x1-x2)**2+(y1-y2)**2+(z1-z2)**2)/2
